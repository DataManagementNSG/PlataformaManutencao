...
def enviar_email_quantidade_minima_atingida(spare, responsavel_email):
    ...
    subject = 'Alerta de quantidade mínima atingida para: {}'.format(spare.nome)
    ...
    if instance.quantidade > 0:
        try:
            ...
            if instance.quantidade > spare.quantidade:
                raise ValidationError(f"Tentativa de registrar uma saída com quantidade maior do que o estoque disponível para o material {spare.nome}.")
            else:
